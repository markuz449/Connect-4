...
impl Game {
    ...
    pub fn valid_check(game: &Game, player_choice: usize) -> bool {
        1 <= player_choice
            && player_choice < (game.board.len() + 1)
            && game.board[player_choice - 1][0] == 0
    }
    ...
    ///
    /// ``` rust
    /// horiz_count = horhoriz_countizCount + Game::token_count(&self, player_choice, y_position,  1,  0,  0);
    ...
    pub fn board_check(&mut self, player_choice: usize, y_position: usize) {
        ...
        let mut diag_count_1: i8 = 0;
        let mut diag_count_2: i8 = 0;
        ...
        diag_count_1 = diag_count_1 + Game::token_count(&self, player_choice, y_position, -1, 1, 0);
        diag_count_1 =
            diag_count_1 + Game::token_count(&self, player_choice, y_position, 1, -1, -1);
        diag_count_2 = diag_count_2 + Game::token_count(&self, player_choice, y_position, 1, 1, 0);
        diag_count_2 =
            diag_count_2 + Game::token_count(&self, player_choice, y_position, -1, -1, -1);

        // Checks to see if player has beaten the game
        if horiz_count >= 4 || verti_count >= 4 || diag_count_1 >= 4 || diag_count_2 >= 4 {
    ...
    ///
    /// ``` rust
    /// pos_x;       // This is current x position that is being checked
    /// pos_y;       // This is current y position that is being checked
    /// x_pos_change; // This is a singular number which tells the direction of change on the x axis: 1 for right, -1 for left, 0 for no change
    /// y_pos_change; // This is a singular number which tells the direction of change on the y axis: 1 for up, -1 for down, 0 for no change
    ...
    ///
    /// ``` rust
    /// let x_bound_check: i8 = pos_x as i8;
    /// if (x_bound_check + x_pos_change < 0){
    ///     return count += 1;
    /// }
    /// ```
    ///
    ...
    pub fn token_count(
        ...
        if (x_bound_check + x_pos_change < 0 || y_bound_check + y_pos_change < 0)
            || (x_bound_check + x_pos_change >= 7 || y_bound_check + y_pos_change >= 6)
        {
            count += 1;
            return count;
        }
...
mod tests {
    ...
    fn test_find_placement() {
        let mut _game = Game::new_game();
        let y_pos = Game::find_placement(&_game, 6);
        assert_eq!(y_pos, 5);
    }
